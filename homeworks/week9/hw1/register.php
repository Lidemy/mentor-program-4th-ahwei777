...
<body>
  ...
  <nav>
    ...
    <div class = 'nav-btns'>
      ...
      <a class = "active" href = "register.php">註冊</a>
      <a href = "login.php">登入</a>
    </div>
  </nav>
  <section class = "wrapper">
    <h1 class = 'title'>註冊</h1>
    <form class = "form_register check" method = "POST" action = "handle_register.php">
      <!--  註冊錯誤返回顯示訊息  -->
      <?php if (!empty($_GET['errCode'])) {
        $errCode = $_GET['errCode'];
        if ($errCode == 3) {
          echo "<div class = 'warning'>此帳號已有人使用!</div>";
        } 
      } else {
        echo "<div class = 'hide'>你看不到我</div>";
      }
      ?>
      <!--  結束  -->
